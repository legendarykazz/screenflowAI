...
function checkWatchTogether() {
  ...
  assertIncludes(join, "showSyncedWatchPlayer ? 'Watch Together'", 'Participant labels synchronized media separately from host presentation streams');
  assertIncludes(join, '<WatchTogetherPlayer', 'Participant renders the synchronized Watch Together source');
  assertIncludes(join, "previewWatch === 'web'", 'Development builds can preview the participant website presentation layout');
  assertIncludes(join, "const [showSelfView, setShowSelfView] = useState(true);", 'Participants can see their own camera tile by default');
  ...
  assertIncludes(player, '<webview', 'Desktop Watch Together uses an embedded browser surface');
  assertIncludes(player, 'capturePage()', 'Embedded browser frames are captured for the live presentation');
  ...
  assertIncludes(player, "pointerEvents: controller ? 'auto' : 'none'", 'Participants cannot independently control embedded web pages');
  assertNotIncludes(player, 'Open shared website in new tab', 'Watch Together does not push guests to an external browser');
  ...
  assertIncludes(live, 'drawEmbeddedBrowserFrame', 'Presenter captures embedded web pages for viewers');
  assertIncludes(live, 'browserOutputCanvasRef', 'Presenter publishes embedded browser frames from a dedicated canvas');
  assertIncludes(live, 'setStatus(`${source.label} is live inside ScreenFlow.`)', 'Web links open inside the presenter app');
  assertIncludes(join, "watchSession.kind === 'web' && hasHostScreen", 'Guests prefer the host browser stream over an independent page');
  assertNotIncludes(live, "window.open(source.url, '_blank', 'noopener,noreferrer')", 'Website Watch Together links do not open a system browser');
  assertNotIncludes(live, 'await window.electron.openExternal(source.url)', 'Desktop Watch Together websites stay inside the app');
...
function checkElectronBridge() {
  ...
  assertIncludes(preload, 'supportsEmbeddedBrowser: true', 'Preload announces the embedded browser capability');
  ...
  assertIncludes(main, 'webviewTag: true', 'Electron enables the in-app Watch Together browser');
  assertIncludes(main, 'will-attach-webview', 'Electron hardens attached Watch Together browser views');
